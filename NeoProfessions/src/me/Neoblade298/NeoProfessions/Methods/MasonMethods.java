...
public class MasonMethods {
	...
	static final int ENGRAVE_ESSENCE_PER_LVL = 12;
		...
		if (p.hasPermission("mason.engrave.tier." + level)) {
			...
			if(numSlots >= MAX_SLOTS) {
				if (p.hasPermission(("mason.engrave.max."  + (numSlots + 1)))) {
					if(p.getInventory().containsAtLeast(CommonItems.getEssence(level), ENGRAVE_ESSENCE_PER_LVL * level)) {
						if(econ.has(p, ENGRAVE_GOLD_BASE + (ENGRAVE_GOLD_PER_LVL * level))) {
							MasonUtils.createSlot(item, level);
							p.getInventory().removeItem(Util.setAmount(CommonItems.getEssence(level), ENGRAVE_ESSENCE_PER_LVL * level));
